...
def re_tsne(jsdata, lbl, classification, dataset):
    ...
    for i in range(0,len(jsdata)):
        current = jsdata[i]
        if (int(current['class']) == int(classification) and int(current['label']) == int(lbl) and int(current['confirmed']) == 0):
...
def class_ssim_csv(dataset, label, clas):
    return send_file(data_location + dataset + '/ssim/ssim_' + str(label) + '_' + str(clas) + '.csv',
                     mimetype='text/csv',
                     attachment_filename='train_images.csv',
                     as_attachment=True)
...
def image(dataset, part, label, image):
    image_location = data_location + dataset + '/' + part + '/' + label + '/' + image
...
def modify_csv(dataset, lbl, classification):
    ...
    test_writer.writerow(['image', 'label', 'class', 'percentage', 'name', 'probabilities', 'confirmed', 'distance_grey', 'distance_hue', 'distance_saliency_hue', 'tsne_unprocessed', 'tsne_saliency'])
    image_number = 0
    while image_number < len(jsdata):
        ...
        test_writer.writerow([image, label, clas, percentage, name, probabilities, confirmed, distance_grey, distance_hue, distance_saliency_hue, tsne_unprocessed, tsne_saliency])
